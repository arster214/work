...
def get_arm_state_callback2(msg):
    ...
    global ARM_DOF
    joint_values = list(msg.joint)
    if len(joint_values) != ARM_DOF:
        rospy.logwarn("joint size mismatch: expect %d, got %d", ARM_DOF, len(joint_values))

    joint_str = ", ".join(["{:.6f}".format(v) for v in joint_values])
    rospy.loginfo("joint state is: [%s]", joint_str)
