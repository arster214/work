...
void callback_servoMove(const servo_ros::ServoMove& msg){
    ...
    s_buffer[3] = 0x03;
    s_buffer[4] = 0x01;
    // 使用消息中的时间参数，如果为0则默认1000ms
    uint16_t move_time = (msg.time > 0) ? msg.time : 1000;
    s_buffer[5] = move_time & 0xFF;       // 时间低八位
    s_buffer[6] = (move_time >> 8) & 0xFF; // 时间高八位
    s_buffer[7] = msg.servo_id;  // 舵机ID
    s_buffer[8] = msg.angle & 0xFF; // 角度位置低八位
    s_buffer[9] = (msg.angle >> 8) & 0xFF; // 角度位置高八位
    ros_ser.write(s_buffer, sBUFFERSIZE);
    // ROS_INFO_STREAM("Control Servo Move: ID=" << msg.servo_id << " Angle=" << msg.angle << " Time=" << move_time);
}
