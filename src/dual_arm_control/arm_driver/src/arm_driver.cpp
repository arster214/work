...
void Movep_Fd_Callback(const dual_arm_msgs::CarteFdPose msg)
{
    // ROS_INFO("enter Movep_Fd_Callback");
    int res = 0;
    byte speed;
    POSE target;
    target.px = msg.Pose_Euler[0];
    target.py = msg.Pose_Euler[1];
    target.pz = msg.Pose_Euler[2];
    target.rx = msg.Pose_Euler[3];
    target.rx = msg.Pose_Euler[4];
    target.rx = msg.Pose_Euler[5];
    // target = Quater_To_Euler(msg.Pose);
    res = Movep_CANFD(target);
    joint_flag = true;
    if (res == 0)
...
/*****************************设置Udp参数配置***************************/
void Set_Realtime_Push_callback(const dual_arm_msgs::Set_Realtime_Push msg)
{
    int res = 0;
    uint16_t cycle;
    bool enable;
...
bool read_data()
{
    ...
    //     oss << std::hex << std::setw(2) << std::setfill('0') << (static_cast<unsigned int>(udp_socket_buffer[i]) & 0xFF) << " ";
    // }
...
void udp_timer_callback(const ros::TimerEvent)
{
    if(read_data()==true)
    {
        //ROS_INFO("UDP data received, ready to parse");
        int parse_result = Parser_Udp_Msg(udp_socket_buffer);
        //ROS_INFO("Paese_result: %d", parse_result);
        if(parse_result == 0)
        {
            //ROS_INFO("successfully parsed UDP data");
        }
        else
        {
            ROS_ERROR("Parse failed, data not processed");
        }
    }
    else
    {
        udp_failed_time++;
        ROS_WARN("UDP read failed, count: %d", udp_failed_time);
...
int main(int argc, char **argv)
{
    ...
    dual_arm_msgs::Six_Force zero_force;
    dual_arm_msgs::Six_Force work_zero_force;
    dual_arm_msgs::Six_Force tool_zero_force;
    std_msgs::Bool state;
    dual_arm_msgs::Joint_Current joint_current;
    dual_arm_msgs::ArmState armState;
    dual_arm_msgs::LiftState liftState;
    dual_arm_msgs::Arm_Software_Version arm_version;

    /***********************************UDP控制数据类型*************************************/ 
    dual_arm_msgs::Set_Realtime_Push udp_set_realtime_push;

    geometry_msgs::Quaternion quaternion_msg;
    tf2::Quaternion quaternion_tf;

    private_nh_.param<std::string>("Arm_IP",               Arm_IP_,                "192.168.10.18");
    ...
    private_nh_.param<std::string>("Udp_IP",               Udp_IP_,                "192.168.10.10");
    ...
    else if(arm_dof == 7)
    {
        ...
        /***** ********************************START***************************************
     * 20211207修改: 从launch文件中读取arm_joints数组,启动多个该节点(如双臂)时匹配具体的关节名称
     * *********************************************************************************/
    ...
    Get_Arm_Software_Version_Result = nh_.advertise<dual_arm_msgs::Arm_Software_Version>("rm_driver/Get_Arm_Software_Version_Result", 1);
    /********************************************udp配置参数修改控制********************************************/
    ...
    Get_Realtime_Push_Result = nh_.advertise<dual_arm_msgs::Set_Realtime_Push>("rm_driver/Get_Realtime_Push_Result", 1);
    /********************************************力传感器外力数据********************************************/
    ...
    pub_JointErrorCode = nh_.advertise<dual_arm_msgs::Manual_Set_Force_Pose>("rm_driver/JointErrorCode", 100);
    /*****************************************发布当前的受力基准坐标系*******************************************/
    ...
    // publisher
    ChangeTool_Name = nh_.advertise<dual_arm_msgs::ChangeTool_State>("rm_driver/ChangeTool_State", 1);
    ChangeWorkFrame_Name = nh_.advertise<dual_arm_msgs::ChangeWorkFrame_State>("rm_driver/ChangeWorkFrame_State", 1);
    ArmCurrentState = nh_.advertise<dual_arm_msgs::Arm_Current_State>("rm_driver/Arm_Current_State", 10);
    pub_armCurrentState = nh_.advertise<dual_arm_msgs::ArmState>("rm_driver/ArmCurrentState", 10);
    ...
    pub_Force_Position_State = nh_.advertise<dual_arm_msgs::Force_Position_State>("rm_driver/Force_Position_State", 10);
    ...
    Arm_IO_State = nh_.advertise<dual_arm_msgs::Arm_IO_State>("rm_driver/Arm_IO_State", 1);
    Tool_IO_State = nh_.advertise<dual_arm_msgs::Tool_IO_State>("rm_driver/Tool_IO_State", 1);
    Plan_State = nh_.advertise<dual_arm_msgs::Plan_State>("rm_driver/Plan_State", 1);
    pub_PoseState = nh_.advertise<geometry_msgs::Pose>("rm_driver/Pose_State", 1);
    pub_PoseEuler = nh_.advertise<dual_arm_msgs::Arm_Pose_Euler>("rm_driver/Udp_Pose_Euler", 1);
    pub_currentJointCurrent = nh_.advertise<dual_arm_msgs::Joint_Current>("rm_driver/Joint_Current", 1);
    pub_liftState = nh_.advertise<dual_arm_msgs::LiftState>("rm_driver/LiftState", 1);
    ...
    /*************************************六维力数据发布（非UDP）*******************************************/
    pub_GetSixForce = nh_.advertise<dual_arm_msgs::Six_Force>("rm_driver/GetSixForce", 100);
    pub_SixZeroForce = nh_.advertise<dual_arm_msgs::Six_Force>("rm_driver/SixZeroForce", 100);
    pub_Work_Zero_Force = nh_.advertise<dual_arm_msgs::Six_Force>("rm_driver/WorkZeroForce", 100);
    pub_Tool_Zero_Force = nh_.advertise<dual_arm_msgs::Six_Force>("rm_driver/ToolZeroForce", 100);
    /*************************************六维力数据发布（UDP）*******************************************/
    pub_UdpSixForce = nh_.advertise<dual_arm_msgs::Six_Force>("rm_driver/UdpSixForce", 100);
    pub_UdpSixZeroForce = nh_.advertise<dual_arm_msgs::Six_Force>("rm_driver/UdpSixZeroForce", 100);
            ...
            if ((temp[0] == 0x0A) && (buffer_cnt > 1))
            {
                ...
                if (msg == 0x0D)
                {
                    ...
                    case FORCE_POSITION_STATE:
                        real_joint.header.stamp = ros::Time::now();
                        ROS_INFO("Preparing to publish joint_states");
                        for (i = 0; i < 6; i++)
                        {
                        ...
                        ROS_INFO("Publishing joint_states");
                        Joint_State.publish(real_joint);
                        Info_Arm_Err();
                        break;
                    case GET_SIX_FORCE:
