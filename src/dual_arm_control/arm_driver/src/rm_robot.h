...
#include <dual_arm_msgs/Arm_Software_Version.h>
#include <dual_arm_msgs/Arm_Pose_Euler.h>
...
#include <dual_arm_msgs/CarteFdPose.h>
...
#include <dual_arm_msgs/Set_Realtime_Push.h>
...
dual_arm_msgs::Arm_Pose_Euler udp_arm_euler;
dual_arm_msgs::Six_Force Udp_Six_Force;
dual_arm_msgs::Six_Force Udp_Six_Zero_Force;
dual_arm_msgs::Gripper_Set arm_joint_error;
std_msgs::UInt16 udp_arm_error;
std_msgs::UInt16 udp_sys_error;
std_msgs::UInt16 udp_coordinate;
dual_arm_msgs::Manual_Set_Force_Pose udp_joint_error_code;
...
int Parser_Arm_Pose(char *msg)
{
    ...
    if (result != NULL && result->type == cJSON_Array)
    {
        ...
        if (size == 6)
        {
            for (i = 0; i < size; i++)
            {
                pose[i] = cJSON_GetArrayItem(result, i)->valuedouble;
            }
            ...
            if (result == NULL)
            {
                result = cJSON_GetObjectItem(root, "err");
            }
            ...
            if (result != NULL)
            {
                ...
                else if (result->type == cJSON_Array)
                {
                    ...
                    {
                        arm_err = cJSON_GetArrayItem(result, 0)->valueint;
                    }
                }
                // ROS_INFO("******************arm_err: %d********************", arm_err);
            }
            ...
            if (result == NULL)
            {
                result = cJSON_GetObjectItem(root, "joint_cur_pos");
            }
...
int Parser_Arm_Current_State(char *msg)
{
    ...
    if (joint == NULL) {
        joint = cJSON_GetObjectItem(arm_state, "joint_cur_pos");
    }

    pose = cJSON_GetObjectItem(arm_state, "pose");

    if (joint != NULL && joint->type == cJSON_Array)
    {
        int j_size = cJSON_GetArraySize(joint);
        for (i = 0; i < 6 && i < j_size; i++)
        {
            Arm_Current_State.joint[i] = cJSON_GetArrayItem(joint, i)->valuedouble;
        }
        if (arm_dof == 7 && j_size > 6)
        {
            Arm_Current_State.joint[6] = cJSON_GetArrayItem(joint, 6)->valuedouble;
        }
    }

    if (pose != NULL && pose->type == cJSON_Array)
    {
        int p_size = cJSON_GetArraySize(pose);
        for (i = 0; i < 6 && i < p_size; i++)
        {
            Arm_Current_State.Pose[i] = cJSON_GetArrayItem(pose, i)->valuedouble;
        }
    }

    arm_err = cJSON_GetObjectItem(arm_state, "arm_err");
    if (arm_err == NULL) {
        arm_err = cJSON_GetObjectItem(arm_state, "err");
    }
    ...
    {
        if (arm_err != NULL) {
            if (arm_err->type == cJSON_Number) {
                Arm_Current_State.arm_err = arm_err->valueint;
            } else if (arm_err->type == cJSON_Array && cJSON_GetArraySize(arm_err) > 0) {
                Arm_Current_State.arm_err = cJSON_GetArrayItem(arm_err, 0)->valueint;
            }
        }
    }

    if (sys_err != NULL && sys_err->type == cJSON_Number) {
...
int Parser_Realtime_Arm_Joint_State(char *msg)
{
    ...
    // =========================================================
    // 1. 获取关节角度 (修复了键名错误的致命BUG)
    // 原代码: joint_position -> 新代码: joint_cur_pos
    // =========================================================
    json_member = cJSON_GetObjectItem(result, "joint_cur_pos");
    // Fallback to "joint_position" if "joint_cur_pos" is missing
    if (json_member == NULL) {
        json_member = cJSON_GetObjectItem(result, "joint_position");
        if (json_member != NULL) {
             //ROS_INFO("Found 'joint_position' (fallback)");
        }
    } else {
         ROS_INFO("Found 'joint_cur_pos'");
    }
    ...
    if (json_member != NULL && json_member->type == cJSON_Array)
    {
        ...
        // 确保不越界，取最小值
        int count = (size < arm_dof) ? size : arm_dof;
        ...
        if (count > 0)
        {
            ...
            {   
                ...
                {
                    // ---------------------------------------------------
                    // 修复了整数除法精度丢失的问题 (int/int = int)
                    // 必须先转 float 再除以 1000.0
                    // ---------------------------------------------------
                    float raw_val = (float)json_sub->valueint;
                    
                    // 存入结构体 (单位: 度)
                    Udp_RM_Joint.joint[i] = raw_val / 1000.0f;
            ...
            // ！！！关键点！！！
            // 只有发布了这个，TF树才会更新，Octomap才有数据
            Joint_State.publish(udp_real_joint);
            //ROS_INFO("Published Joint_State");
        ...
        ROS_WARN("No joint data found in joint_status (checked joint_cur_pos and joint_position)");
    }
    // 如果没找到角度数据，记录警告但继续执行（为了健壮性）
    // else { ROS_WARN("joint_cur_pos not found"); }


    // =========================================================
    // 2. 获取关节电流 (尝试兼容不同固件版本)
    // =========================================================
    json_member = cJSON_GetObjectItem(result, "joint_cur_cur"); // 新版协议
    if(json_member == NULL) 
        json_member = cJSON_GetObjectItem(result, "joint_current"); // 旧版协议兼容
    ...
    if (json_member != NULL && json_member->type == cJSON_Array)
    {
        ...
        for (i = 0; i < count; i++)
        {
            ...
            if(json_sub) {
                // 同样进行精度修复
                Udp_RM_Joint.joint_current[i] = (float)json_sub->valueint / 1000.0f;
            }
        }
    }
    ...
    if (json_member != NULL && json_member->type == cJSON_Array)
    {
        ...
        for (i = 0; i < count; i++)
        {
            json_sub = cJSON_GetArrayItem(json_member, i);
            if(json_sub) {
                Udp_RM_Joint.en_flag[i] = (uint16_t)json_sub->valueint;
            }
        }
    }
    ...
    if (json_member != NULL && json_member->type == cJSON_Array)
    {
        ...
        for (i = 0; i < count; i++)
        {
            json_sub = cJSON_GetArrayItem(json_member, i);
            if(json_sub) {
                Udp_RM_Joint.err_flag[i] = (uint16_t)json_sub->valueint;
    ...
    // 5. 末端位姿解析（位置 + 姿态）
    // =========================================================
    cJSON *waypoint = cJSON_GetObjectItem(root, "waypoint");
    if(waypoint != NULL)
    {
        cJSON *pos = cJSON_GetObjectItem(waypoint, "position");
        // 兼容键名 "pos" 或 "position"
        if(pos == NULL) pos = cJSON_GetObjectItem(waypoint, "pos");

        cJSON *euler = cJSON_GetObjectItem(waypoint, "euler");
        cJSON *quat = cJSON_GetObjectItem(waypoint, "quat");

        bool has_pose = false;
        bool has_orientation = false;

        if(pos != NULL && (pos->type == cJSON_Array) && cJSON_GetArraySize(pos) >= 3)
        {
            // 假设单位是 0.001mm (即 1um)，除以 1000000 变为米
            // 如果你在Rviz发现位置不对，请检查这里的系数是 1000 还是 1000000
            double scale = 1000000.0;

            Udp_RM_Joint.joint_position[0] = (float)cJSON_GetArrayItem(pos, 0)->valueint / scale;
            Udp_RM_Joint.joint_position[1] = (float)cJSON_GetArrayItem(pos, 1)->valueint / scale;
            Udp_RM_Joint.joint_position[2] = (float)cJSON_GetArrayItem(pos, 2)->valueint / scale;
            ...
            udp_arm_euler.Px = udp_arm_pose.position.x;
            udp_arm_euler.Py = udp_arm_pose.position.y;
            udp_arm_euler.Pz = udp_arm_pose.position.z;
            has_pose = true;
        }

        if(quat != NULL && (quat->type == cJSON_Array) && cJSON_GetArraySize(quat) >= 4)
        {
            // 四元数一般是 1e-6 精度
            double q_scale = 1000000.0;
            udp_arm_pose.orientation.x = (double)cJSON_GetArrayItem(quat, 0)->valueint / q_scale;
            udp_arm_pose.orientation.y = (double)cJSON_GetArrayItem(quat, 1)->valueint / q_scale;
            udp_arm_pose.orientation.z = (double)cJSON_GetArrayItem(quat, 2)->valueint / q_scale;
            udp_arm_pose.orientation.w = (double)cJSON_GetArrayItem(quat, 3)->valueint / q_scale;
            has_orientation = true;
        }

        if(euler != NULL && (euler->type == cJSON_Array) && cJSON_GetArraySize(euler) >= 3)
        {
            // 欧拉角一般为 0.001rad 精度
            double rpy_scale = 1000.0;
            double roll = (double)cJSON_GetArrayItem(euler, 0)->valueint / rpy_scale;
            double pitch = (double)cJSON_GetArrayItem(euler, 1)->valueint / rpy_scale;
            double yaw = (double)cJSON_GetArrayItem(euler, 2)->valueint / rpy_scale;

            udp_arm_euler.Rx = roll;
            udp_arm_euler.Ry = pitch;
            udp_arm_euler.Rz = yaw;
            ...
            if(!has_orientation)
            {
                ...
                udp_arm_pose.orientation.x = q_msg.x;
                udp_arm_pose.orientation.y = q_msg.y;
                udp_arm_pose.orientation.z = q_msg.z;
                udp_arm_pose.orientation.w = q_msg.w;
                has_orientation = true;
            }
        }
        ...
        if(has_pose)
        {
            ...
            if(has_orientation)
            {
                pub_PoseEuler.publish(udp_arm_euler);
            }
        }
    }
...
int Parser_Udp_Msg(char *msg)
{
    ...
    //ROS_INFO("Start parsing UDP JSON message");
    ...
    // ==========================================
    // Fix Part 1: Parse arm_err / err
    // Robot protocol usually sends "err": [0] (array)
    // ==========================================
    item = cJSON_GetObjectItem(root, "err");
    if (item != NULL && (item->type == cJSON_Array))
    {
        // Get the 0th element of the array
        cJSON *err_val = cJSON_GetArrayItem(item, 0);
        if(err_val != NULL) {
            arm_err = err_val->valueint;
            //ROS_INFO("Parsed arm_err (from array): %d", arm_err);
    ...
    // Compatibility with old protocol: if not found above, try to find "arm_err" (number)
    else 
    {
        item = cJSON_GetObjectItem(root, "arm_err");
        if (item != NULL && item->type == cJSON_Number) {
            arm_err = item->valueint;
            //ROS_INFO("Parsed arm_err (from number): %d", arm_err);
            udp_arm_error.data = arm_err;
            pub_ArmError.publish(udp_arm_error);
        }
    }
    ...
    // ==========================================
    // Handle sys_err (if any)
    // ==========================================
    item = cJSON_GetObjectItem(root, "sys_err");
    if (item != NULL && item->type == cJSON_Number)
    {
        sys_err = item->valueint;
        ROS_INFO("Parsed sys_err: %d", sys_err);
        udp_sys_error.data = sys_err;
        pub_SysError.publish(udp_sys_error);
    }
  
    // ==========================================
    // Fix Part 2: Enter joint data parsing
    // ==========================================
    item = cJSON_GetObjectItem(root, "joint_status");
    if (item != NULL)
    {
        //ROS_INFO("Found joint_status, calling Parser_Realtime_Arm_Joint_State");
        // Call sub-function to parse specific joint data
        // Note: Must use the fixed version of Parser_Realtime_Arm_Joint_State provided below
        ...
        if (res == 0)
        {
            //ROS_INFO("Parser_Realtime_Arm_Joint_State success");
            return 0; // Success
        }
        else
        {
            ROS_ERROR("Parser_Realtime_Arm_Joint_State failed with code: %d", res);
            return -3; // Internal parsing failure
        }
    }

    ROS_WARN("No joint_status found in JSON");
    // If no joint_status, it might be a heartbeat packet or other
    cJSON_Delete(root);
    return 5;    
}
