...
def build_marker_array(result: ProcessingResult, frame_id: str, raw_points: Optional[np.ndarray] = None, show_raw_voxels: bool = True) -> MarkerArray:
    ...
    if result.table_plane:
        ...
        arr.markers.append(make_cube_marker(next_id, 'table_plane', frame_id, plane.center, size, color(0.1, 0.7, 0.2, 0.18)))
        next_id += 1

    for obstacle in result.obstacles:
        arr.markers.append(make_cube_marker(next_id, 'obstacle_aabb', frame_id, obstacle.center, obstacle.size, color(0.85, 0.2, 0.2, 0.28)))
        next_id += 1
