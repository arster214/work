...
int main(int argc, char** argv) {
    ...
    double x, y, z;
    // The input file format is expected to be x y z (optional extra columns ignored)
    string line;
    long count = 0;
    
    while (infile >> x >> y >> z) {
        // Skip the rest of the line (e.g. size or other attributes)
        getline(infile, line); 
        
        point3d endpoint((float)x, (float)y, (float)z);
        // updateNode adds the node to the tree and marks it as occupied
